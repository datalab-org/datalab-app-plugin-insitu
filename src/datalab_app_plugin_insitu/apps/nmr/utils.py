    ...
    df = pd.DataFrame(
        {
            ...
            "original_exp_number": [int(x) for x in exp_numbers],
        }
    )
...
def prepare_for_bokeh(
    ...
    ppm_values = nmr_data["ppm"].values

    sampled_nmr_data = nmr_data
    ...
    if echem_df is not None and not echem_df.empty:
        echem_time = (echem_df["time/s"] / 3600).values
        voltage = echem_df["Voltage"].values

        result["echem"] = {
            "Voltage": voltage.tolist(),
            "time": echem_time.tolist(),
        }
...
def _process_data(
    ...
    try:
        ...
        if nmr_dimension == "1D":
            spec_paths, acqu_paths, exp_numbers = setup_paths(
